...
export class PharmacophoreListItem extends React.Component<IProps, {}> {
  ...
  renderDropdown() {
    ...
    const showProtein =
      p.protein && p.protein.visible && p.protein.proteinVisible
    return (
      <DropdownButton pullRight={true} bsSize="small" id={p.id} title="">
        ...
        >
          <Glyphicon
            glyph={p.pharmacophore.visible ? 'eye-open' : 'eye-close'}
          />
          &nbsp; Pharmacophore
        </MenuItem>
        <MenuItem
        ...
        <MenuItem
          ...
          disabled={
            !(p.visible && (p.ligand || (p.protein && p.protein.hasHetero)))
          }
          onSelect={this.onLigandVisibilityClick}
        >
          <Glyphicon glyph={showLigand ? 'eye-open' : 'eye-close'} />
          &nbsp; Ligand
        </MenuItem>
        <MenuItem
          title={
            p.protein && p.protein.pocketVisible ? 'Hide pocket' : 'Show pocket'
          }
          disabled={
            !(
              p.visible &&
        ...
        >
          <Glyphicon
            glyph={
              p.protein && p.protein.pocketVisible ? 'eye-open' : 'eye-close'
            }
          />
          &nbsp; Pocket
        </MenuItem>
        ...
        <MenuItem
          title={
            p.pharmacophore.solid ? (
              'Transparent pharmacophore'
            ) : (
              'Solid pharmacophore'
            )
          }
