...
const fetchWhenServerModelChangedEpic: Epic<epicActions, IState> = (action$, store) =>
    action$.ofType(SERVER_MODEL_CHANGED).mergeMap<epicActions, epicActions>(() => {
...
const fetchServerHiLiteChangedEpic: Epic<epicActions, IState> = (action$, store) =>
    ...
    action$.ofType(SERVER_HILITE_CHANGED).mergeMap(() => {
        switch (store.getState().sse.currentPage) {
            case 'LigandsAndProteinsViewer':
                return Observable.from([
                    ligands.hiLitefetchRequested()
                ]);
            case 'PharmacophoresViewer':
                return Observable.from([
                    pharmacophores.hiLitefetchRequested()
                ]);
            default:
                return Observable.from([]);
        }
